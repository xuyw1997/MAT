...
def save_checkpoint(root_path, model, optimizer, epoch, cfg, scaler=None, comment=''):
    """
    Save a checkpoint.
    Args:
    ...
    if not du.is_master_proc(cfg.DDP.NUM_GPUS * cfg.NUM_SHARDS):
        return
    # Ensure that the checkpoint dir exists.
    pathmgr.mkdirs(get_checkpoint_dir(root_path))
    # Omit the DDP wrapper in the multi-gpu setting.
    sd = model.module.state_dict() if cfg.DDP.NUM_GPUS > 1 else model.state_dict()
