...
class ActivityNetCaptionsClip(Dataset):
    ...
    glove_vocab_size = len(vocab.itos)
    print("glove_vocab_size",glove_vocab_size)
    def __init__(self, cfg, split):
        super(ActivityNetCaptionsClip, self).__init__()
        self.cfg = cfg
        self.split = split
        ...
        with open(os.path.join(cfg.DATA.ANNO_PATH, f'{split}.json'),'r') as f:
            annotations = json.load(f)

        with open(cfg.DATA.VOCAB_PATH, 'r') as f:
            tmp = json.load(f)
            self.itos = tmp['words']
        assert len(self.itos) == cfg.DATA.VOCAB_SIZE
        self.stoi = {}
        ...
        for vid, video_anno in annotations.items():
            ...
            for timestamp, sentence in zip(video_anno['timestamps'], video_anno['sentences']):
                if timestamp[0] < timestamp[1]:
                    ...
                    desc_split = sentence.split()
                    # desc_split, txt_mask = utils.trim_pad_to_fixed_length(desc_split, self.num_max_word)
                    # txt_mask = torch.ones(min(len(desc_split), self.num_max_word))
    ...
    def __getitem__(self, index):
        ...
        # if len(desc_split) > self.num_max_word:
        #     desc_split = desc_split[:self.num_max_word]
        ...
        word_idxs = torch.tensor([self.vocab.stoi.get(w.lower(), self.glove_vocab_size - 1) for w in desc_split], dtype=torch.long)
    ...
    def get_video_frames(self, video_id, frame_seq):
        ...
        for idx in frame_seq:
            images.append(Image.open(os.path.join(self.data_path, video_id, f'{video_id}_{idx+1:06}.jpg')).convert('RGB'))
