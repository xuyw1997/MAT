...
class TACoS(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if video_id == 'pad_video':
            ...
            frame_seq = utils.get_frame_sequence(self.cfg, self.num_video_frame[f'{video_id[:-4]}-cam-002'], rand_offset=self.split == 'train')
            frame_seq = frame_seq[clip_id * self.window_size: (clip_id + 1) * self.window_size]
            frames = self.get_video_frames(f'{video_id[:-4]}-cam-002', frame_seq)
