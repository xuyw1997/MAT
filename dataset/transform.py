...
class I3DToTorchFormatTensor(object):
    """ Converts a PIL.Image (RGB) or numpy.ndarray (H x W x C) in the range [0, 255]
    to a torch.FloatTensor of shape (C x H x W) in the range [0.0, 1.0] """
    def __init__(self, div=True):
        self.div = div
    ...
    def __call__(self, pic):
        if isinstance(pic, np.ndarray):
            ...
            # yikes, this transpose takes 80% of the loading time/CPU
            img = img.transpose(0, 1).transpose(0, 2).contiguous()
        return (img.float()/ 255.0) * 2.0 - 1.0
        ...
        for i in range(n):
            for fn_id in fn_idx:
                if fn_id == 0 and brightness_factor is not None:
                    img_group[i] = F.adjust_brightness(img_group[i], brightness_factor)
                elif fn_id == 1 and contrast_factor is not None:
                    img_group[i] = F.adjust_contrast(img_group[i], contrast_factor)
                elif fn_id == 2 and saturation_factor is not None:
                    img_group[i] = F.adjust_saturation(img_group[i], saturation_factor)
                elif fn_id == 3 and hue_factor is not None:
                    img_group[i] = F.adjust_hue(img_group[i], hue_factor)
